...
class BCMP_MVA:
    ...
    def getMVA(self):
            ...
            for n in range(self.N): #P336 (8.43)
                for r in range(self.R):
                    if self.type_list[n] == 3:
                        ...
                        if m[n] == 1: #P336 (8.43) Type-1,2,4 (m_i=1)
        ...
        L_index = {'class0': self.L[:,0,last], 'class1' : self.L[:,1,last]}
        df_L = pd.DataFrame(L_index)
        df_L.to_csv('./csv/L(N:'+str(self.N)+',R:'+str(self.R)+',K:'+str(self.K)+').csv')
        return self.L[:,:,last]
    ...
    def getArrival(self, p):#マルチクラスの推移確率からクラス毎の到着率を計算する
        ...
        for r in range(R): #マルチクラス毎取り出して到着率を求める
