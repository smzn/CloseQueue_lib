...
class BCMP_Simulation:
    ...
    def __init__(self, N, R, K, mu, type_list, p, time):
    ...
    def getSimulation(self):
        ...
        #最初の拠点を均等にする(人数が多いとき、最初のノードからさばくのに時間がかかる)
        for i in range(R): #クラス
            for j in range(K[i]): #各クラスの人数
                #各拠点に順に配分
                self.event[initial_node].append("arrival") #ノードへの到着
                ...
                initial_node += 1
                initial_node %= self.N
        for n in range(self.N): #各ノードの先頭にサービス時間を設定
            if queue[n] > 0:
                service[n] = self.getExponential(self.mu[n]) #先頭客のサービス時間設定
        ...
        pd.DataFrame(L).to_csv('/content/drive/MyDrive/研究/BCMP/csv/L(N:'+str(self.N)+',R:'+str(self.R)+',K:'+str(self.K)+',Time:'+str(self.time)+').csv')
        pd.DataFrame(Lc).to_csv('/content/drive/MyDrive/研究/BCMP/csv/Lc(N:'+str(self.N)+',R:'+str(self.R)+',K:'+str(self.K)+',Time:'+str(self.time)+').csv')
        pd.DataFrame(Q).to_csv('/content/drive/MyDrive/研究/BCMP/csv/Q(N:'+str(self.N)+',R:'+str(self.R)+',K:'+str(self.K)+',Time:'+str(self.time)+').csv')
        pd.DataFrame(Qc).to_csv('/content/drive/MyDrive/研究/BCMP/csv/Qc(N:'+str(self.N)+',R:'+str(self.R)+',K:'+str(self.K)+',Time:'+str(self.time)+').csv')
        rmse_index = {'time': rmse_time, 'RMSE': rmse}
        df_rmse = pd.DataFrame(rmse_index)
        df_rmse.to_csv('/content/drive/MyDrive/研究/BCMP/csv/RMSE(N:'+str(self.N)+',R:'+str(self.R)+',K:'+str(self.K)+',Time:'+str(self.time)+').csv')
...
if __name__ == '__main__':
    ...
    theoretical = pd.read_csv('/content/drive/MyDrive/研究/BCMP/csv/Theoretical33_K1000.csv')
    time = 10001
